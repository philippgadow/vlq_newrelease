...
def inc_child(particles, particle_id):
    particles[particle_id]['nchild'] += 1
    if particles[particle_id]['parent0'] == 0:
        return
    if particles[particle_id]['parent0'] == particles[particle_id]['parent1'] :
        inc_child(particles, particles[particle_id]['parent0'])
    else:
        inc_child(particles, particles[particle_id]['parent0'])
        inc_child(particles, particles[particle_id]['parent1'])
...
def particlemapper(_list):
    ...
    for ii in range(1,len(_list)):
        ...
        if particlemap['status'] == 1:
            if particlemap['parent0'] == particlemap['parent1']:
                inc_child(particles, particlemap['parent0'])
            else:
                inc_child(particles, particlemap['parent0'])
                inc_child(particles, particlemap['parent1'])
...
def fix_list(particlelist, VLQMode, fermchild, bosonchild):
    ...
    def not_in_decay_chain(particle):
        ...
        ### light quarks/leptons coming from W or Z boson
        if abs(particle['id']) in [1, 2, 3, 4, 11, 12, 13, 14, 15, 16] and particle['status'] == 1 \
           and particles[particle['parent0']]['id'] == particles[particle['parent1']]['id']\
           and abs(particles[particle['parent0']]['id']) in [23, 24] and particles[particle['parent0']]['nchild'] == 2:
            return False

        ### light quarks/leptons coming from top quark ( W propagator missing)
        if abs(particle['id']) in [1, 2, 3, 4, 11, 12, 13, 14, 15, 16] and particle['status'] == 1 \
           and particles[particle['parent0']]['id'] == particles[particle['parent1']]['id']\
           and abs(particles[particle['parent0']]['id']) == 6 and particles[particle['parent0']]['nchild'] == 3:
            return False

        ### b quarks from t quark
        if abs(particle['id']) == 5 and particle['status'] == 1 \
           and particles[particle['parent0']]['id'] == particles[particle['parent1']]['id']\
           and abs(particles[particle['parent0']]['id']) == 6:
            return False

        ### b quark from Z boson
        if abs(particle['id']) == 5 and particle['status'] == 1 \
           and particles[particle['parent0']]['id'] == particles[particle['parent1']]['id']\
           and abs(particles[particle['parent0']]['id']) == 23 and particles[particle['parent0']]['nchild'] == 2:
            return False

        ### W boson from t quark
        if abs(particle['id']) == 24 and particle['status'] == 2 \
           and particles[particle['parent0']]['id'] == particles[particle['parent1']]['id']\
           and abs(particles[particle['parent0']]['id']) == 6:
            return False
        ...
        # #    return True
        # if abs(particles[particle['parent0']]['id']) in [bosonchild,6] and abs(particles[particle['parent1']]['id']) in [bosonchild,6]:
        # #and abs(particles[particles[particle['parent0']]['parent0']]['id']) == VLQMode:  # children of boson/fermion from VLQ decay
        #     return False
    ...
    for key in sorted(particles.keys()):
        ...
        if this_particle['parent0'] == 0 and this_particle['parent1'] == 0: 
            continue
        this_list = particlelist[key].strip().split()
        if abs(this_particle['id']) == bosonchild and this_particle['status'] == 2 and this_particle['nchild'] == 2: ## Decayed Bosonic child (W, Z, H)
            #and (abs(particles[this_particle['parent0']]['id']) == VLQMode or abs(particles[this_particle['parent1']]['id']) == VLQMode) \
    ...
    for key in sorted(particles.keys()):
        if particles[key]['keepit'] == False:
            ...
            for _key in sorted(particles.keys()):
                ...
                if particles[_key]['parent1'] > key: 
                    particles[_key]['parent1'] = particles[_key]['parent1'] - 1
